...
<title>Home</title>
 ...
 <link rel="validation" href="js/scheduler.js">
...
<nav class="navigation">
   ...
   <a href="Login.jsp"><button class="btnlogin_popup">Login</button></a> 
	...
	<form action="Saveservlet" method="post" onsubmit="return validateStartDate()">
    ...
    <br>
    Duration (in hours): <input type="text" name="duration" required><br>
   
        
        <label for="batchType">Batch Type:</label>
        <input type="radio" id="weekday" name="batchType" value="weekday" onchange="autofillStartDate()" required>
        <label for="weekday">Weekday</label>
        <input type="radio" id="weekend" name="batchType" value="weekend" onchange="autofillStartDate()" required>
        <label for="weekend">Weekend</label><br>
   
     <label for="startDate">Start Date:</label>
        <input type="date" id="startDate" name="startDate" required><br>
    Hours Per Day: <input type="text" name="hoursPerDay"><br>
    <input type="submit" value="Submit">
	  ...
	  <script>
        function validateStartDate() {
            ...
            var batchType = document.querySelector('input[name="batchType"]:checked').value;
            var selectedDate = new Date(startDateInput.value);

            if (batchType === "weekday" && (selectedDate.getDay() === 0 || selectedDate.getDay() === 6)) {
                alert("Selected date must be a weekday for the chosen batch type.");
                return false; // Prevent form submission
            } else if (batchType === "weekend" && (selectedDate.getDay() !== 0 && selectedDate.getDay() !== 6)) {
                alert("Selected date must be a weekend for the chosen batch type.");
                return false; // Prevent form submission
            }

            // If the date is valid, set the action attribute and submit the form
            document.getElementById("courseForm").action = "NextPageServlet"; // Replace with your actual URL
            return true;
        }
    </script>
</body>
